...
5. **本地自检**：成稿后跑 `python3 scripts/gates/run.py --trigger=adapter --legacy=check-sourcing --requirement <REQ-ID>`，期望退出码 = 0；有 error 必须修复后再进入下一阶段（`/requirement:next` 门禁会再跑一次）
