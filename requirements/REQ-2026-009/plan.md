...
- **Decision**：(1) **save_review.py signoff 入口语义保留**（兼容期内不变）；(2) **内部委托 signoff.py:run_signoff()**；(3) **save_review.py 顶层 import signoff**（lazy access via module-level alias）+ **signoff.py 函数体内 lazy import save_review**（解循环）；(4) **save_review.py 缩到 423 行（< 500）/ signoff.py 497 行（接近 500 上限，下次大改前考虑进一步拆 helper 到 save_review_validation.py）**。
