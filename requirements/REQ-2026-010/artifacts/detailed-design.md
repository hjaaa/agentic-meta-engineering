...
def dispatch_node(
    ...
    try:
        ...
        if "skill" in node:                         return _dispatch_skill_node(node, run_state, env, jsonl_path)
        if "prompt" in node or "prompt_file" in node: return _dispatch_prompt_node(node, run_state, env, run_dir, root, jsonl_path)
        if "bash" in node:                          return _dispatch_bash_node(node, run_state, env, run_dir, root, jsonl_path)
        if "approval" in node:     return _dispatch_approval_node(node, env, run_state, jsonl_path)
