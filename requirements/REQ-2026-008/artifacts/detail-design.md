...
| 6 | 3 份 schema 的 schema_version + SUPPORTED_VERSIONS | ⏳ 后续 |
| 7 | F-007 派发模板 frontmatter `touches` | ⏳ 后续 |
| 8 | 4 个新 gate 的 applies_when 字段 | ⏳ 后续 |
...
下一批次推进顺序候选：**#6 + #8 + #9 + #12 + #13**（schema 演化窗口 + gate applies_when + 文档收口三连）→ **#7 + #10**（派发模板 + 合并复评）。
