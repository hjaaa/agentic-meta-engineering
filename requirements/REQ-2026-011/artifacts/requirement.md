...
- **status / doctor 能力薄弱**：当前 `/workflow:status` 输出粗粒度，不区分 ready / running / blocked / paused；无 heartbeat / stale 检测，半写 jsonl 或卡在 approval pending 时缺诊断手段。
...
设计留档 `context/team/engineering-spec/specs/2026-05-13-workflow-runtime-dag-completion-design.md`（沿用既有命名 `YYYY-MM-DD-<topic>-design.md`，来源：context/team/engineering-spec/specs/INDEX.md）由阶段 4-5 概要/详细设计阶段产出，**不作为本 requirement 开发验收标准的一部分**——后续设计阶段闭环。
...
- 主流程：
  ...
  5. 外层 `dispatch_node` 进入即写 `node_started` → `_dispatch_artifact_node` 调 `run_artifact_checks.run_artifact_checks()` 校验 `must_exist` 列表 → 校验通过后写 `node_completed`（失败由外层 try/except 统一转 `node_failed`，handler 不重复写 started）
...
- 期望结果：approval pending → completed → next node ready，全过程在 jsonl 可追溯。**reject 路径**（与 AC-03b 一致的 inline repair 事件流）：reject CLI 写 `approval_rejected` + `approval_repair_started(attempt=N)` + 状态机切到 `awaiting_claude_action` → 主 Claude 执行 `approval.on_reject.prompt` 完成修订 → 调 `save_node_result.py --kind=approval_repair` 写 `approval_repair_completed` → 状态回 `approval_pending` 等下一次 approve/reject；未达 max_attempts 时不写 `node_failed`，达上限才 `node_failed` + `workflow_failed`。
...
- 主流程：
  1. 第一个进程取 `runs/.locks/<run-id>.lock`（requirement 类 run 自动 symlink 到 `requirements/.locks/<req-id>.lock`） `fcntl.LOCK_EX`
...
- **兼容性**（已确认延续 D-007 双轨）：
  ...
  - 旧 `current_node.next` 字段保留为兼容路径：**判断依据是原始 YAML 是否显式声明 `depends_on`**——loader 当前 `_expand_implicit_depends_on` 会给缺省节点自动补 `[prev_id]`（来源：scripts/lib/workflow_loader.py:475），导致加载后 `depends_on` 永远非空。本需求需在该函数内补 `depends_on_explicit: bool` 标记位（缺省 = False），scheduler 据此区分"显式 DAG"与"隐式链"两条路径，避免兼容路径在加载后被一刀切跳过。
- **安全/合规**：
...
| AC | 内容 | 验证手段 | 档位 |
|---|---|---|---|
| AC-01 | DAG `depends_on` ready-node scheduler：(a) loader `_expand_implicit_depends_on` 内补 `depends_on_explicit: bool` 标记位（来源：scripts/lib/workflow_loader.py:475）；(b) `_next_node` 升级为 `_ready_nodes`，按 Kahn 拓扑层从已完成事件推算 ready 集；(c) 退化兼容路径**基于 `depends_on_explicit=False` 触发**而非 `depends_on==[]`，回落到 `current_node.next`。**同层多 ready 节点本期落地策略**：识别 ready 集合 + 按 yaml 出现顺序**串行派发**（保持 `RunState` 单 `current_node` 模型，事件顺序确定可复现）；multi-Agent 同响应并发派发列入"不包含"，由后续需求覆盖 | e2e：(i) 起一个仅用 `depends_on` 的最小 yaml（≥ 5 节点），跑过两层；(ii) 起一个仅用 `next` 的最小 yaml，跑通退化路径；(iii) `code-review-embedded.yaml` 8 个 checker 被识别为同层 ready 并按 yaml 顺序串行写入 jsonl | P0 |
| AC-02 | `_dispatch_artifact_node` 实现：(a) 直接调用统一入口 `scripts/lib/run_artifact_checks.run_artifact_checks()`（来源：scripts/lib/run_artifact_checks.py:145），已支持 must_exist / must_not_exist / schema_check / must_contain_sections / must_match_regex 五类；**事件写入职责收紧**：外层 `dispatch_node` 已统一写 `node_started` 与异常分支 `node_failed`（来源：scripts/lib/workflow_dispatcher.py:128），handler **只负责 success path 写 node_completed**，不重复写 node_started，failure 由外层 try/except 自动转 node_failed；(b) **附带修正既有 YAML 中不存在的脚本引用**：`.claude/workflows/requirement/standard-8phase.yaml:57` 引的 `scripts/lib/check_meta_schema.py` 在仓库内不存在，改为已存在的 `scripts/lib/check_meta.py` | e2e：`standard-8phase.yaml` 的 `bootstrap-validate` artifact 节点跑通且 schema_check 命中真实脚本；jsonl 中 node_started/node_completed 各 1 条（不双写） | P0 |
| AC-03 | approval 闭环 + on_reject 修订循环：**AC-03a** approve 后写当前 approval 节点 `node_completed` + 推进下游；**AC-03b** reject 后**不立即把 approval 节点写 `node_failed`**——`on_reject` 在真实 yaml 里是 `approval.on_reject.prompt` inline 子结构（来源：.claude/workflows/requirement/standard-8phase.yaml:156），不是独立 DAG node。事件流：reject CLI 原子写两条事件 `approval_rejected` + `approval_repair_started(attempt=N, prompt_ref)`**并把状态机从 `approval_pending` 切换到 `awaiting_claude_action`**（与 AC-04 AI 节点等待外部动作语义复用同一状态，避免引入 `approval_repairing` 第二个 awaiting 状态）→ 主 Claude Code 执行 inline `approval.on_reject.prompt` 完成修订 → 调 `save_node_result.py --kind=approval_repair --run=<id> --node=<approval-node-id>` 写 `approval_repair_completed` → 状态机回 `approval_pending` 等待下一次 approve/reject；**AC-03c** attempt 计数（默认上限 N=3 [待用户确认]，可由 yaml `approval.on_reject.max_attempts` override，来源：.claude/workflows/requirement/standard-8phase.yaml:168），**达上限**时由 reject CLI 写 approval 节点 `node_failed` + `workflow_failed` 且 reason=`approval_attempts_exhausted`，下游节点保持未派发（scheduler 看到 approval 节点 failed 后不进 ready 集） | e2e：AC-03a pending → approve → next node ready；AC-03b pending → reject → jsonl 含 approval_rejected + approval_repair_started(attempt=1) + RunState.state=`awaiting_claude_action` → `save_node_result --kind=approval_repair` → approval_repair_completed + 状态回 approval_pending；AC-03c 三次 reject 后 jsonl 末尾出现 approval 节点 node_failed + workflow_failed 且 reason=`approval_attempts_exhausted` | P0 |
| AC-04a | AI 节点完成契约（派发侧）：`skill/prompt/agent` dispatcher 写 `node_ready` + 状态机入 `awaiting_claude_action`；新增 `scripts/lib/save_node_result.py`（已确认接口路径）由主 Claude Code 调用写 `node_completed` 并把状态机回 `running` | e2e：派发 skill 节点 → jsonl 含 node_ready 且 RunState.state=`awaiting_claude_action` → 调 `python3 scripts/lib/save_node_result.py --run=<id> --node=<id> --output=<json>` → 含 node_completed 且 state=`running` | P1 |
| AC-04b | AI 节点完成契约（状态机/命令侧同步）：(i) `run_state.py` 新增 `node_ready` / `approval_repair_started` / `approval_repair_completed` 进 `VALID_EVENT_TYPES` 并把 `awaiting_claude_action` 接入 `RunState.rebuild` 派生表（来源：scripts/lib/run_state.py:89）：`node_ready` 与 `approval_repair_started` 两类事件均映射到 `awaiting_claude_action` 状态；`node_completed` / `approval_repair_completed` 分别回 `running` / `approval_pending`；(ii) `workflow_state_validator.CMD_ALLOWED_STATES` 把 `awaiting_claude_action` 加入 `continue / save / status / cancel / rollback` 的允许集（来源：scripts/lib/workflow_state_validator.py:29）；(iii) **`/workflow:continue` 在 `awaiting_claude_action` 下行为 = 只读 / 不派发**：识别当前节点是 awaiting → print `INFO: node <id> awaiting external save_node_result (kind=skill_result\|approval_repair), no dispatch` → return 0；不写任何新事件、不推进 ready 集，避免重复派发（**与 fail-closed 二选 idempotent 路径**）；(iv) `/workflow:status` 输出能展示该状态；(v) `/workflow:cancel` 在该状态下走 graceful 路径（不残留半执行 jsonl）；(vi) `save_node_result.py` 通过 `--kind=skill_result\|approval_repair` 子模式区分写入的事件名（`skill_result` → `node_completed`；`approval_repair` → `approval_repair_completed`），**两种 kind 均要求 RunState.state == `awaiting_claude_action`**，state ≠ `awaiting_claude_action` 时 fail-closed 拒绝写入并退出码 2 | unit：状态矩阵 5 命令对 `awaiting_claude_action` 各跑一遍允许/拒绝断言；状态派生表对 `node_ready` / `approval_repair_started` 两条新事件断言映射到 `awaiting_claude_action`；e2e：与 AC-04a 同 yaml，分别跑 `continue`（断言只输出 INFO 不派发）/ `status` / `cancel` / 异常 `save_node_result`（state=running 时拒绝）四条路径；交叉用例：AC-03b reject → `save_node_result --kind=approval_repair` 在 awaiting 下成功写入并状态回 approval_pending | P1 |
...
- **同层多 ready 节点的并发派发**（multi-Agent 同响应启多个 subagent，code-review-embedded.yaml:18-19 注释意图）：本期仅做 ready 集合识别 + 串行派发（见 AC-01），并发派发涉及 `RunState.current_node` 从单值改为集合 / 事件交错处理 / TaskCreate 多通道协调，列入后续需求 follow-up。
...
| 兼容退化判定依据 | A: 加载后 `depends_on==[]`；B: 原始 YAML 是否显式声明 `depends_on`（loader 内加 `depends_on_explicit` 标记位） | **B** | loader `_expand_implicit_depends_on` 已会给缺省节点自动补 `[prev_id]`（scripts/lib/workflow_loader.py:475），A 方案在加载后实际永不命中；显式标记位保留原始语义、最小侵入 |
| 同层 ready 节点本期执行策略 | A: 识别 + 串行派发（保 `RunState` 单 `current_node`）；B: 识别 + 并发派发（改 `RunState.current_node` 为集合）；C: 返回 ready 集合给 Claude Code 分批执行 | **A** | 本期目标"能闭环"为先；B 涉及 `RunState` 模型重写 + 事件交错 + TaskCreate 多通道，与 REQ-010 改动叠加风险大；并发能力列入后续需求 |
| `artifact` dispatcher 复用层 | A: 调用 `scripts/lib/run_artifact_checks.run_artifact_checks()` 统一入口；B: 单挑 `check_meta.py` / `check_sourcing.py` 等脚本；C: 写新 `_dispatch_artifact_node` 内部 must_exist 逻辑 | **A** | `run_artifact_checks.py:145` 已是 5 类校验（must_exist / must_not_exist / schema_check / must_contain_sections / must_match_regex）的统一入口；schema_check 子项继续走 `check_meta.py` 等已存在脚本，避免双实现 |
...
| on_reject 事件流模型 | A: 把 `on_reject` 视为独立 DAG node，reject 即写 approval 节点 `node_failed` + 跳到 on_reject node；B: 把 `approval.on_reject.prompt` 当 approval 节点子结构（真实 yaml 形态，来源：.claude/workflows/requirement/standard-8phase.yaml:156），reject 写 synthetic 事件 `approval_repair_started/completed`，max_attempts 内回 `approval_pending`，达上限才 `node_failed + workflow_failed` | **B** | A 在 DAG scheduler 模型下会把下游 permanently blocked（approval 节点 failed 即不再进 ready 集）或提前触发失败矩阵 retry；B 与 yaml 真实结构一致，事件流自描述、attempts 计数与 yaml `max_attempts` 同源 |
| approval_repair 中间态归属 | A: 复用 `awaiting_claude_action`（AC-04 同款），`save_node_result --kind=approval_repair` 在该状态合法；B: 新增独立 `approval_repairing` 状态，仅 `--kind=approval_repair` 在该状态合法；C: 修订期间仍保持 `approval_pending`，`save_node_result` 例外放行 | **A** | 语义统一："等主 Claude 完成外部动作 + 调 save_node_result" 即 `awaiting_claude_action` 的定义；B 增加状态机维度（CMD_ALLOWED_STATES 多一列、RunState.rebuild 多一类事件映射）成本无收益；C 破坏 `save_node_result` 的 fail-closed 单一状态校验语义、给 AI agent 绕过留口 |
| `awaiting_claude_action` 状态下 continue 语义 | A: fail-closed 拒绝（CMD_ALLOWED_STATES 不含该状态）；B: 允许 + 只读（识别 awaiting → INFO 提示 → 不派发不写事件）；C: 允许 + 自动重派发 | **B** | A 对脚本调用方过严，用户用 continue 试探状态会被误拦；C 必然破坏 save_node_result 单一写入语义；B 保留 continue 的 idempotent 语义、不污染 jsonl |
...
| DAG scheduler feature flag | A 引入 env 双跑期 / B 不引入靠 `next` 退化 / C meta.yaml per-run 开关 | **B 不引入**（用户已确认） | 已有兼容路径（`depends_on_explicit=False` → 退化 `next`，见上方"兼容退化判定依据"决策），加 flag 反增清理成本 |
...
17. ✅ **D-01 AC-02 artifact dispatcher 复用入口**：定稿 `scripts/lib/run_artifact_checks.run_artifact_checks()` 统一入口（来源：scripts/lib/run_artifact_checks.py:145）；附带修正 `standard-8phase.yaml:57` 引的不存在脚本 `check_meta_schema.py` → `check_meta.py`。
...
20. ⏳ **D-04 AC-01 同层 ready 并发派发** [follow-up 需求]：本期定为串行（见决策记录"同层 ready 节点本期执行策略"），并发派发能力（`RunState.current_node` 集合化 + 事件交错 + TaskCreate 多通道）拉一个 REQ-XXXX 独立处理；code-review-embedded.yaml 8 checker 在本期以"串行 8 次派发"完成闭环。
