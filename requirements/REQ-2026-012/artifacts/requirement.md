...
- **AC-5（`make ci-local`）**：在 clean checkout 后跑 `make ci-local` 退出 0 = 等价于 CI `quality-check` job 全过（含 hyperfine 路径、`pytest --ignore=tests/benchmarks/`）；途中任一 step fail → make 退非 0 + 明确指明失败 step
