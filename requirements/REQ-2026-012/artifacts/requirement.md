...
2. **旧 hook 测试指向已删除文件**：旧 hook shell 测试在 `.claude/hooks/tests/` 下原以 `HOOK=".claude/hooks/protect-branch.sh"` 形式硬编码已删除路径——该 2 个脚本已于 F-002 commit d7624d2 删除（删前内容可由 `git show d7624d2~1:.claude/hooks/tests/test_protect-branch.sh` 取回）；该分支保护逻辑现已搬到 pre-tool-use-guard.sh 的 Edit/Write/MultiEdit 分支，调用 check_branch_protect 函数（来源：.claude/hooks/pre-tool-use-guard.sh:101）。
